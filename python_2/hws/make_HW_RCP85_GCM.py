...
tasmax_his_ifile   = f'tasmax_{MODELS[M]}_his_{RES}.nc'
tasmax_rcp85_ifile = f'tasmax_{MODELS[M]}_rcp85_{RES}.nc'
tasmax_p90_ifile = f'tasmax_p90_{MODELS[M]}_{RES}.nc' 
lsmask_ifile     = f'lsmask_{MODELS[M]}_{RES}.nc'

cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_GCM}/{filename_his[M]}', output=tasmax_his_ifile)
cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_GCM}/{filename_rcp85[M]}', output=tasmax_rcp85_ifile)
...
f_out = f'{root_dir}/scripts/python2/hw/outputs_post_prague/rcp85/GCM/{filename_rcp85[M]}'
f_out = f'{root_dir}/scripts/python_2/hws/outputs_post_prague/rcp85/GCM/{filename_rcp85[M]}'

# tasmax his
dataset = Dataset(f'{tasmax_his_ifile}', mode='r')
...
tasmax_his = dataset.variables['tasmax'][:]
dataset.close()

# tasmax rcp85
dataset = Dataset(f'{tasmax_rcp85_ifile}', mode='r')
lon     = dataset.variables['lon'][:]
lat     = dataset.variables['lat'][:]
if np.min(lon)>=0 : 
    lon_lsm = lon - 180  
...
date    = num2date(tim[:],dataset['time'].units)
units   = dataset['time'].units
tasmax_rcp85 = dataset.variables['tasmax'][:]
dataset.close()
...
dataset = Dataset(f'{DIR_GCM_out}/{filename_his[0]}', mode='r')
...
HW_persistence       = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_intensity         = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_index_event_start = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_index_event_end   = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_HWMI              = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_mean_tmax         = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_max_tmax          = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
HW_number            = np.empty([len(YEARS),tasmax_rcp85.shape[1],tasmax_rcp85.shape[2]])*np.nan
...
for Y in range(YSTART,YEND,1) : 
    ...
    cc = cc + 1 
    # Count heatwave  
    tasmax_y  = cdo.selyear(Y,input='-selmonth,6/8 '+f'{tasmax_rcp85_ifile}',returnCdf=True).variables['tasmax'][:]     
    #point_his = da_tasmax_yr.where((np.round(da_tasmax_yr.lon,2)==11.25) & (np.round(da_tasmax_yr.lat,2)==46.93),drop=True).squeeze()
...
os.remove(tasmax_his_ifile)
os.remove(tasmax_rcp85_ifile)
...
cdo.yearmean(input = f'{root_dir}/GCMs/{MODELS[M]}/surface/{filename_rcp85[M]}',output = 'ofile.nc',options = '-f nc') # Attention to the time_period from one model only
