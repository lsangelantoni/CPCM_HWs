...
DIR_CP     = f'{root_dir}/FILES_CP/tasmax/historical'
DIR_CP_hfls= f'{root_dir}/FILES_CP/hfls/historical'
...
if filename[M] == 'tasmax_ALP-3_HadGEM3-GC3.1-N512_historical_r1i1p1_HadREM3-RA-UM10.1_fpsconv-x0n1-v1_day_JJA.nc' :
    YSTART = 1998
    YEND = 2008

elif filename[M] == 'tasmax_ALP-3_CNRM-CERFACS-CNRM-CM5_historical_r1i1p1_CNRM-AROME41t1_fpsconv-x2yn2-v1_day_JJA.nc' : 
    YSTART = 1996
    YEND = 2005
else : 
    YSTART = 1996
...
f_out = f'/data/cmcc/ls21622/gcm_driven_experiment_from_nird/scripts/python_2/hws/TMP_TO_TEST_SIMILARITY/{filename[M]}'
...
if filename[M] == 'tasmax_ALP-3_CNRM-CERFACS-CNRM-CM5_historical_r1i1p1_CNRM-AROME41t1_fpsconv-x2yn2-v1_day_JJA.nc' :
    ntim = ntim -1  
...
if MODELS[M] == 'CNRM' : 
    cdo.yearmean(input = f'{DIR_CP_hfls}/{filename_hfls[3]}',    output = 'ofile.nc', options = '-f nc') 
else : 
    cdo.yearmean(input = f'{DIR_CP}/{filename[0]}',    output = 'ofile.nc', options = '-f nc')
...
ds = xr.Dataset(
    ...
    data_vars=dict(
         ...
         HW_hfls_deficit     = (["time","x", "y"], dataset.variables['HW_hfls_deficit'][:]),
         HW_mean_hfls        = (["time","x", "y"], dataset.variables['HW_mean_hfls'][:]),
