...
DIR_nonCP_his     = f'{root_dir}/FILES_nonCP/tasmax/historical'
DIR_nonCP_rcp85   = f'{root_dir}/FILES_nonCP/tasmax/rcp85'
DIR_nonCP_p90     = f'{root_dir}/FILES_nonCP/tasmax/historical/p90'
DIR_nonCP_his_hfls   = f'{root_dir}/FILES_nonCP/hfls/historical'
DIR_nonCP_rcp85_hfls = f'{root_dir}/FILES_nonCP/hfls/rcp85'
DIR_nonCP_p90     = f'{root_dir}/FILES_nonCP/tasmax/historical/p90'
...
tasmax_ifile_his   = f'tasmax_his_{MODELS[M]}_{RES}.nc'
tasmax_ifile_rcp85 = f'tasmax_rcp85_{MODELS[M]}_{RES}.nc'
tasmax_p90_ifile   = f'tasmax_p90_{MODELS[M]}_{RES}.nc' 
hfls_ifile_his     = f'hfls_his_{MODELS[M]}_{RES}.nc'
hfls_ifile_rcp85   = f'hfls_rcp85_{MODELS[M]}_{RES}.nc'
tasmax_y_ifile     = f'tasmax_{MODELS[M]}_Y_{RES}.nc'
hfls_y_ifile       = f'hfls_{MODELS[M]}_Y_{RES}.nc'


# Load variables 
cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_nonCP_his}/{filename_his[M]}', output=tasmax_ifile_his)
cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_nonCP_rcp85}/{filename_rcp85[M]}', output=tasmax_ifile_rcp85)
...
cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_nonCP_his_hfls}/{filename_hfls_his[M]}', output=hfls_ifile_his)
cdo.sellonlatbox(lon_min,lon_max,lat_min,lat_max, input=f'{DIR_nonCP_rcp85_hfls}/{filename_hfls_rcp85[M]}', output=hfls_ifile_rcp85)
...
for Y in range(YSTART,YEND,1) : 
    ...
    if MODELS[M] == 'BTU' or MODELS[M] == 'KIT' :
        cdo.selyear(Y+1, input=f'{hfls_ifile_rcp85}', output=hfls_y_ifile)
    else : 
        cdo.selyear(Y, input=f'{hfls_ifile_rcp85}', output=hfls_y_ifile)
...
cdo.yearmean(input = f'{DIR_nonCP_rcp85}/{filename_rcp85[0]}',    output = 'ofile.nc', options = '-f nc') # Attention to the time_period from one model only
